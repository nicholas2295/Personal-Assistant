...
When found: marks them read and fires run_brief.sh to generate the daily brief.
...
BRIEF_SCRIPT = os.path.expanduser("~/.daily-brief-scripts/run_brief.sh")
...
def fire_briefing() -> bool:
    """Run the daily brief via run_brief.sh (same script launchd uses at 8:30 AM)."""
    log.info("Firing daily briefing via run_brief.sh...")
    result = subprocess.run(["/bin/bash", BRIEF_SCRIPT])
